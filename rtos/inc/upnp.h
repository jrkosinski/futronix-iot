...
bool upnp_init_server();
void upnp_send(struct udp_pcb *upcb, const ip_addr_t *addr, u16_t port);
void upnp_handle(
...
/**
  ...
  */
struct udp_pcb* mcast_join_group(char *group_ip, uint16_t group_port, void (* recv)(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, u16_t port))
{
    bool status = false;
    struct udp_pcb *upcb;
...
/**
  ...
  * @retval None
  */
void upnp_callback(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
    if (p) {
...
bool upnp_init()
{
    struct udp_pcb *upcb = mcast_join_group(UPNP_MCAST_GRP, UPNP_MCAST_PORT, upnp_callback);
    return (upcb != NULL);
}
