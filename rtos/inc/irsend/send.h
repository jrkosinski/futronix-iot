...
void ir_enable_out(uint32_t khz, uint8_t duty); 
void ir_send_data(uint16_t onemark, uint32_t onespace, uint16_t zeromark, uint32_t zerospace, uint64_t data, uint16_t nbits, bool MSBfirst);
void ir_mark(uint32_t time);
void ir_space (uint32_t time); 
void ir_off(); 
uint32_t ir_calc_us_period(uint32_t hz, bool use_offset); 
...
void ir_enable_out(uint32_t freq, uint8_t duty) {
/*
...
void ir_mark(uint32_t time) {
  	...
  	if(loops==0) loops=1;
  	for (i=0;i<loops;i++) {
		gpio_write(IR_SEND_PIN, IR_ON); //Set GPIO12 LO -> IR On
		sdk_os_delay_us(13);  //1M/38K/2
		gpio_write(IR_SEND_PIN, IR_OFF); //Set GPIO12 HI -> IR Off
	  	sdk_os_delay_us(13);  //1M/38K/2
  	}
}
